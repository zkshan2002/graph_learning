...
class MAGNN_ctr_ntype_specific(nn.Module):
    def __init__(self,
        ...
        for i in range(num_metapaths):  # etypes_list = [[0, 1], [0, 2, 3, 1], [0, 4, 5, 1]]
    ...
    def forward(self, inputs):
        if self.use_minibatch:
            ...
            for g, edge_metapath_indices, target_idx, metapath_layer in zip(
                    g_list, edge_metapath_indices_list, target_idx_list, self.metapath_layers):

                out = metapath_layer((g, features, type_mask, edge_metapath_indices, target_idx))
                out = F.elu(out.view(-1, self.num_heads * self.out_dim))
        ...
        beta = []
        ## fc is node type conditioned
        ## use same fc because here all metapath start nodes share a same node type
